...
# MAGIC %md
# MAGIC
# MAGIC #### This notebook loads data from the onprem sql table and write it to landing container daily based on last_inserted_datetime in config json file.
...
# MAGIC  2. process_landing_incremental_load (This is the main method that will be triggered at daily)<br/>
# MAGIC      a. Read the config file(json) for the table located in Configuration folder<br/>
    ...
    try:
        ...
        for params in parameter_sets[table_name]:
            ...
            # Set widget values
                ...
                #create_folders_if_not_exist(landing_table_path, folders) (HelperFunctions)
                ...
                last_inserted_datetime = get_last_inserted_datetime(rc_to_landing_df,parameter_values['watermark_column'])
        ...
        print(f"On-prem to landing flow: Exception occured in process_landing_incremental_load, table_name={table_name}, error={str(e)}")
        sendLog(f"On-prem to landing flow: Exception occured in process_landing_incremental_load, table_name={table_name}, error={str(e)}")
...
process_landing_incremental_load("EmployeeInformation")
