...
# MAGIC %md
# MAGIC #### This notebook reads data from the landing container and write it to bronze container at every 30 minutes based on last_inserted_datetime read from config json file.<br />
...
# MAGIC       c. Get the row_count and last_inserted_datetime from the dataframe returned from (b) (HelperFunctions) <br/>
    ...
    with open(f'/Workspace/Shared/Configuration/{table_name}.json', 'r') as json_file:
        ...
        for params in parameter_sets[table_name]:
            ...
            try:
                ...
                print(f"Landing to Bronze flow: Processing failed for {host_name}  with error: {str(e)}")
...
# run_incremental_prod_process(process_bronze_incremental_load)

for table in get_tables_prod_list():
    process_bronze_incremental_load(table)
